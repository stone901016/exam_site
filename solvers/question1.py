...
def solve(file_path):
    ...
    for α in levels:
        ...
        var_hist_amt[k]    = va
        cvar_hist_amt[k]   = ca
        var_hist_rat[k]    = vr * 100
        cvar_hist_rat[k]   = cr * 100
    ...
    ax.hist(sim_amt, bins=50, density=True,
            color="skyblue", edgecolor="gray", alpha=0.7,
            label=f"{best_amt} 模擬樣本 (n={Nsim})")
    ax.set_xlabel("賠償金額")
    ax.set_ylabel("機率密度")
    ax.legend()
    plt.tight_layout()
    ...
    ax = plt.gca()
    ax.grid(True, linestyle="--", alpha=0.5)
    ax.hist(sim_rat, bins=50, density=True,
            color="lightgreen", edgecolor="gray", alpha=0.7,
            label=f"{best_rat} 模擬樣本 (n={Nsim})")
    ax.set_xlabel("賠款率 (%)")
    ax.set_ylabel("機率密度")
    ax.legend()
    plt.tight_layout()
